    ...
    for x in mmocr_data:
        for i in x["predictions"]:
            if i["image path"] == filename:
                ...
                for w in x["score"]:
                    weight += x["score"][w]
                predictions.append({"text": i["prediction"], "weight": weight})
    print(predictions)
...
f.close()
f = open("google results.json","r")
google_data = json.loads(f.read())
f.close()
all_data = mmocr_data
all_data.append(google_data[0])

predictions = []
for i in img_paths:
    predictions.append(detect_text(all_data, i))
...
results.append({"model": "data fusion with google all stats", "score": ocr_scores, "predictions": img_predictions})

f = open("data fusion results with google all stats.json", "w")
